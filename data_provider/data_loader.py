...
import warnings
from data_provider.loadder import load_from_tsfile_to_dataframe_2
...
class MSLSegLoader(Dataset):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        print('MSL dataset for anomaly detection')
        print("test:", self.test.shape)
        print("train:", self.train.shape)
...
class UCRloader(Dataset):
    """
    Dataset class for datasets included in:
        Time Series extrinsic regression Archive
    Argument:
        limit_size: float in (0, 1) for debug
    Attributes:
        all_df: (num_samples * seq_len, num_columns) dataframe indexed by integer indices, with multiple rows corresponding to the same index (sample).
    ...
    def __init__(self, root_path, file_list=None, limit_size=None, flag=None):
        ...
        self.label_mean = self.labels_df.loc[self.all_IDs].values.mean()
        self.label_std = self.labels_df.loc[self.all_IDs].values.std()
    ...
    def load_single(self, filepath):
        # df, labels = load_from_tsfile_to_dataframe(filepath, return_separate_X_and_y=True,
        #                                                      replace_missing_vals_with='NaN')
        df, labels = load_from_tsfile_to_dataframe_2(filepath, return_separate_X_and_y=True,
                                                  replace_missing_vals_with='NaN')
    ...
    def __getitem__(self, ind):
        ...
        return self.instance_norm(torch.from_numpy(self.feature_df.loc[self.all_IDs[ind]].values)), \
               torch.from_numpy((self.labels_df.loc[self.all_IDs[ind]].values-self.label_mean)/(self.label_std+1e-5))
