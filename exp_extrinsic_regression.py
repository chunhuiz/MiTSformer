...
from tools import EarlyStopping, adjust_learning_rate,metric_ext_reg
...
class Exp_Extrinsic_Regression(Exp_Basic):
    def __init__(self, args):
        super(Exp_Extrinsic_Regression, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, task_criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, label, padding_mask) in enumerate(vali_loader):
                ...
                loss = task_criterion(pred, label.cpu())
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, padding_mask) in enumerate(train_loader):
                ...
                pred_out = self.model.Extrinsic_Regression(x_lcv_embed, x_con_embed)
                ...
                task_loss = task_criterion(pred_out, label)
            ...
            if self.args.lradj_flag:
                if (epoch + 1) % 5 == 0:
                    ...
                    adjust_learning_rate(model_optim, int((epoch + 1) / 5) + 1, self.args)
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, label, padding_mask) in enumerate(test_loader):
                ...
                batch_y = label.detach().cpu().numpy().squeeze(-1)
        ...
        preds = np.concatenate(([preds[i] for i in range(len(preds))]), axis=0)
        trues = np.concatenate(([trues[i] for i in range(len(trues))]), axis=0)



        mae_s, rmse_s = metric_ext_reg(preds, trues)
