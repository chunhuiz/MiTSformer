...
import os
import torch
from exp_extrinsic_regression import Exp_Extrinsic_Regression
import random
...
if __name__ == '__main__':
    ...
    # basic config
    parser.add_argument('--task_name', type=str, default='extrinsic_regression',
    ...
    # data loader
    parser.add_argument('--data', type=str, default='UCR', help='dataset type')
    parser.add_argument('--data_name', type=str, default='AustraliaRainfall', help='dataname')
    ...
    if args.data_name =='AppliancesEnergy':
        p=24
        dis_dim=12
    if args.data_name =='HouseholdPowerConsumption1':
        p=5
        dis_dim=2
    if args.data_name =='HouseholdPowerConsumption2':
        p=5
        dis_dim=2
    if args.data_name =='BenzeneConcentration':
        p=8
        dis_dim=4
    if args.data_name =='BeijingPM25Quality':
        p=9
        dis_dim=4
    if args.data_name =='BeijingPM10Quality':
        p=9
        dis_dim=4
    
    if args.data_name =='LiveFuelMoistureContent':
        p=7
        dis_dim=3
    if args.data_name =='AustraliaRainfall':
        p=3
        dis_dim=1
    ...
    if args.data_name =='PPGDalia':
        p=4
        dis_dim=2
    
    
    if args.data_name =='IEEEPPG':
        p=5
        dis_dim=2
    
    
    if args.data_name =='BIDMC32RR':
        p=2
        dis_dim=1
    
    if args.data_name =='BIDMC32HR':
        p=2
        dis_dim=1
    if args.data_name =='BIDMCSpO2':
        p=2
        dis_dim=1
    if args.data_name =='NewsHeadlineSentiment':
        p=3
        dis_dim=1
    if args.data_name =='NewsTitleSentiment':
        p=3
        dis_dim=1
    ...
    args.root_path = './dataset/extrinsic_regression/' + args.data_name

    print('Args in experiment:')
    print_args(args)

    Exp = Exp_Extrinsic_Regression
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            setting = '{}_{}_{}_sl{}_dm{}_ln{}_lradj{}_lr{}_sm_loss_w{}'.format(
                args.task_name,
                args.model,
                args.data_name,
                args.seq_len,
        ...
        exp = Exp(args)  # set experiments
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        mae,rmse = exp.test(setting, test=1)
    ...
    expd_path = './Results_extrinsic_regression_new/' + args.data_name + '/' + args.model + '/'
